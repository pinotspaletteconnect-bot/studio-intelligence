...
export async function POST(request: Request) {
  ...
  const [{ data: credentials, error: credentialError }, { data: account, error: accountError }, { data: targets, error: targetError }] = await Promise.all([
    supabase.rpc("get_meta_account_secret", { p_account_id: parsed.data.accountId }),
    supabase.from("meta_collection_accounts").select("*").eq("account_id", parsed.data.accountId).maybeSingle(),
    supabase.from("meta_collection_targets").select("*").eq("account_id", parsed.data.accountId),
  ])
