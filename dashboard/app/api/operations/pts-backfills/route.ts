...
export async function POST(request: Request) {
  try {
    ...
    const webhookUrl = process.env.PTS_BACKFILL_WEBHOOK_URL
    ...
    const result = await response.json().catch(() => null) as { success?: boolean; rowCount?: number } | null
    ...
    return NextResponse.json({ success: true, rowCount: Number(result.rowCount ?? 0) })
  } catch (error) {
