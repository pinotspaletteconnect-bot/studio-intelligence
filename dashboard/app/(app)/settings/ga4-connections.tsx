...
export function Ga4Connections({ studios, accounts }: { studios: Studio[]; accounts: Account[] }) {
  ...
  return <div className="space-y-5">
    {oauthAccounts.map(account => <div className="space-y-2 rounded-lg border p-3" key={account.id}><div className="flex flex-wrap items-center justify-between gap-2 text-sm"><div><strong>{account.account_name}</strong><p className="text-xs text-muted-foreground">{account.google_account_email ?? "Google account"} · {account.properties.length} {account.properties.length === 1 ? "property" : "properties"}</p></div><div className="flex items-center gap-3"><span className="text-emerald-700">{account.has_credentials ? "OAuth encrypted in Vault" : "Credentials unavailable"}</span><a className="inline-flex h-8 items-center justify-center rounded-md border px-3 text-xs font-medium hover:bg-muted" href={`/api/integrations/ga4/connect?accountId=${account.id}`}>Reconnect</a></div></div>{account.properties.length ? account.properties.map(property => <PropertyMapping key={property.property_id} property={property} studios={studios} />) : <p className="text-xs text-amber-700">No GA4 properties were visible to this Google account.</p>}</div>)}
