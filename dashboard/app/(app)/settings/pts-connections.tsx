...
function PtsConnectionForm({ account }: { account: { id: number; account_name: string; has_credentials: boolean; last_validated_at: string | null } }) {
  ...
  return <form action={action} className="grid gap-4 rounded-lg border p-4 md:grid-cols-2">
    ...
    <div className="md:col-span-2">
      ...
      <p className="mt-1 text-xs text-muted-foreground">{account.has_credentials ? "Username and password are encrypted in Vault and cannot be viewed." : "Enter the PTS username and password below."}{account.last_validated_at ? ` Last validated ${new Date(account.last_validated_at).toLocaleString()}.` : ""}</p>
    </div>
