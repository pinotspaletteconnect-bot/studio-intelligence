...
function PtsConnectionForm({ account }: { account: { id: number; account_name: string; has_credentials: boolean; last_validated_at: string | null } }) {
  ...
  return <form action={action} className="grid gap-4 rounded-lg border p-4 md:grid-cols-2">
    ...
    <div className="md:col-span-2">
      ...
      {account.has_credentials ? <p className="mt-1 text-sm font-medium text-emerald-700">PTS account details loaded in SASHA</p> : <p className="mt-1 text-sm font-medium text-destructive">PTS account details required</p>}
      <p className="mt-1 text-xs text-muted-foreground">{account.has_credentials ? "Update the PTS login by entering the new username and password below. Existing values are encrypted and cannot be viewed." : "Enter the PTS username and password below."}{account.last_validated_at ? ` Last validated ${new Date(account.last_validated_at).toLocaleString()}.` : ""}</p>
    </div>
    ...
    <div className="flex items-center gap-3 md:col-span-2"><Button type="submit" disabled={pending}>{pending ? "Encrypting…" : account.has_credentials ? "Update PTS login" : "Save PTS login"}</Button>{state?.error ? <p role="alert" className="text-sm text-destructive">{state.error}</p> : null}{state?.complete ? <p role="status" className="text-sm text-emerald-600">PTS login saved.</p> : null}</div>
  </form>
}
