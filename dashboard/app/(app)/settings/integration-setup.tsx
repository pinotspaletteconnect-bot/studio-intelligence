...
type Ga4Account = { id: number; account_name: string; has_credentials: boolean; last_discovered_at: string | null; properties: Array<{ account_id: number; property_id: string; display_name: string; account_display_name: string | null; studio_id: number | null; studio_name: string | null }> }
...
export function IntegrationSetup({ ptsAccounts, textellentAccounts, mntnAccounts, eulerityAccounts, ga4Accounts, studios, mappedIntegrationTypes, ptsStudioSettings }: { ptsAccounts: PtsAccount[]; textellentAccounts: TextellentAccount[]; mntnAccounts: MntnAccount[]; eulerityAccounts: EulerityAccount[]; ga4Accounts: Ga4Account[]; studios: Array<{ id: number; studio_name: string }>; mappedIntegrationTypes: string[]; ptsStudioSettings: Array<{ integrationId: number; studioId: number; studioName: string; classpopEnabled: boolean }> }) {
  ...
  return <Card id="integrations">
    ...
    <CardContent className="space-y-3">
      ...
      <Guide title="Google Analytics 4" description="Website traffic, conversions, and attribution." connected={ga4Accounts.some(account => account.has_credentials)} available icon={<BarChart3 className="size-5" />}><div className="grid gap-6 lg:grid-cols-[minmax(0,.8fr)_minmax(0,1.2fr)]"><div><h3 className="mb-3 font-medium">Create reporting access</h3><Steps><li>Create a dedicated service account in Google Cloud and enable the Google Analytics Admin and Data APIs.</li><li>Download its JSON key once.</li><li>In each GA4 property, add the service account&apos;s client email as a Viewer.</li><li>Enter the JSON only in SASHA. Accessible properties are discovered automatically.</li><li>Map every discovered property to its SASHA studio.</li></Steps></div><Ga4Connections studios={studios} accounts={ga4Accounts} /></div></Guide>
