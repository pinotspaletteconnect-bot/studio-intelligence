...
export function HomebaseConnections({ studios, accounts }: { studios: Studio[]; accounts: Account[] }) {
  ...
  return <div className="space-y-5">
    ...
    <form action={action} className="grid gap-4 rounded-lg border p-4 md:grid-cols-2">
      <div className="md:col-span-2"><h3 className="font-medium">Connect a Homebase account</h3><p className="mt-1 text-xs leading-5 text-muted-foreground">Enter the owner login once, then map each SASHA studio to its Homebase location UUID. Credentials are encrypted immediately and never displayed again. SASHA uses the company timesheet view to collect privacy-minimized daily labor totals.</p></div>
