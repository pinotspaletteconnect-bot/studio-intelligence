...
import { StudioAlertForm, TextellentAccountForm, TextellentSenderForm } from "./textellent-settings"
...
export default async function TextellentPage() {
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      {canManage ? <Card><CardHeader><CardTitle>Secure Textellent connections</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">Add each API account once. Multiple studios may share a connection; Jeffersonville should use the St. Matthews connection. Sending numbers remain editable here without exposing the API authentication code.</p>{data.accounts.map(account => <TextellentSenderForm key={account.id} account={account} />)}<TextellentAccountForm /></CardContent></Card> : null}
      <Card><CardHeader><CardTitle>Studio automation</CardTitle></CardHeader><CardContent className="space-y-4">{canManage ? data.studios.map(studio => <StudioAlertForm key={studio.id} studio={studio} accounts={data.accounts} assignment={data.assignments.find(row => row.studio_id === studio.id)} setting={data.settings.find(row => row.studio_id === studio.id)} />) : <p className="text-sm text-muted-foreground">Only owners and administrators can edit these rules.</p>}</CardContent></Card>
