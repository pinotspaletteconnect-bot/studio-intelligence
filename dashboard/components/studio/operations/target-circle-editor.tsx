...
import { parseTargetZipCodes, targetCircleSchema, type TargetCircle, type TargetSettings, type ZipTargets } from "@/lib/maps/target-circles"

type Center = { address: string; latitude: number; longitude: number }
type Settings = TargetSettings & { canEdit: boolean }
type Props = { studioId: number; latitude: number | null; longitude: number | null; circles: TargetCircle[]; onChange: (circles: TargetCircle[]) => void; zipTargets: ZipTargets; onZipChange: (targets: ZipTargets) => void }
...
export function TargetCircleEditor({ studioId, latitude, longitude, circles, onChange, zipTargets, onZipChange }: Props) {
  const prefix = useId()
  ...
  const dirty = saved !== null && (JSON.stringify(saved.circles) !== JSON.stringify(circles) || JSON.stringify(saved.zipTargets) !== JSON.stringify(zipTargets) || Boolean(zipError))
  ...
  useEffect(() => {
    ...
    fetch(`/api/marketing/map-targets?studioId=${studioId}`, { signal: controller.signal, cache: "no-store" })
      ...
      .then(data => { setSaved(data); onChange(data.circles); onZipChange(data.zipTargets); setZipText(data.zipTargets.codes.join(", ")); setZipError(""); setError(""); setNotice("") })
      .catch(error => { if (!controller.signal.aborted) setError(error.message) })
    return () => controller.abort()
  }, [studioId, reload, onChange, onZipChange])
  ...
  async function save() {
    if (!saved || zipError) return
    setBusy(true); setError(""); setNotice("")
    try {
      const response = await fetch("/api/marketing/map-targets", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ studioId, circles, zipTargets, revision: saved.revision }) })
      const body = await response.json(); if (!response.ok) throw new Error(body.error)
      setSaved(body); onChange(body.circles); onZipChange(body.zipTargets); setZipText(body.zipTargets.codes.join(", ")); setNotice("Targets saved for this studio.")
  ...
  return <details className="mt-4 rounded-lg border p-3">
    <summary className="cursor-pointer text-sm font-medium">Map targets · {circles.length} circles · {zipTargets.codes.length} ZIP codes{dirty ? " · Unsaved changes" : ""}</summary>
    <p className="my-2 text-xs text-muted-foreground">Planning overlays only; sales totals and shading are unchanged. Click Save targets before leaving or changing report filters.</p>
    ...
    {!saved ? <p className="text-sm">{error ? <button type="button" className={buttonClass} onClick={() => setReload(n => n + 1)}>Retry loading circles</button> : "Loading saved circles…"}</p> : <>
      ...
      {saved.canEdit ? <>
        <fieldset disabled={busy} className="mt-3 space-y-3 border-t pt-3">
          <legend className="px-1 text-sm font-medium">Target ZIP outlines</legend>
          <label className="block text-sm" htmlFor={`${prefix}-zips`}>Target ZIP codes<textarea id={`${prefix}-zips`} className={inputClass} rows={3} value={zipText} aria-invalid={Boolean(zipError)} aria-describedby={`${prefix}-zip-help`} onChange={event => {
            const text = event.target.value
            setZipText(text); setNotice("")
            try { const codes = parseTargetZipCodes(text); onZipChange({ ...zipTargets, codes }); setZipError("") }
            catch (error) { setZipError(error instanceof Error ? error.message : "Check ZIP codes.") }
          }} placeholder="43201, 43215, 43202" /></label>
          <p id={`${prefix}-zip-help`} className="text-xs text-muted-foreground">Paste up to 200 five-digit ZIP codes separated by commas, spaces or new lines. Duplicates are removed. Clear the list to remove outlines. Valid entries preview immediately; click Save targets to keep them.</p>
          {zipError ? <p role="alert" className="text-sm text-destructive">{zipError} The map still shows your last valid list.</p> : null}
          <label className="block text-sm" htmlFor={`${prefix}-zip-color`}>ZIP border color<input id={`${prefix}-zip-color`} type="color" className="block h-9 w-24 rounded border" value={zipTargets.color} onChange={event => { onZipChange({ ...zipTargets, color: event.target.value }); setNotice("") }} /></label>
          <p className="text-xs text-muted-foreground">Border only—no target fill. Outlines stay above circles and do not change the sales shading.</p>
        </fieldset>
        <fieldset disabled={busy} className="mt-3 space-y-3 border-t pt-3">
        ...
        <div className="mt-3 flex flex-wrap gap-2 border-t pt-3"><button type="button" className={buttonClass} disabled={busy || !dirty || Boolean(zipError)} onClick={save}>{busy ? "Working…" : "Save targets"}</button><button type="button" className={buttonClass} disabled={busy} onClick={() => { if (!dirty || window.confirm("Discard unsaved target changes and reload?")) { resetForm(); setReload(n => n + 1) } }}>Reload saved targets</button></div>
