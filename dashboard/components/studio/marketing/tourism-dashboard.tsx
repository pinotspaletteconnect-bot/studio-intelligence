...
type LocalMarket = { name:string; definition:string; source:string; zipCodes:string[]; excludedOrders:number; excludedSales:number }
type StudioTourism = { studioId:number; studioName:string; homeState:string; knownOrders:number; knownSales:number; outOfStateOrders:number; outOfStateSales:number; orderShare:number; salesShare:number; stateCount:number; unknownOrders:number; localMarket:LocalMarket|null; states:{state:string;orderCount:number;bookedSales:number}[]; zipCodes:{zipCode:string;state:string;orderCount:number;bookedSales:number;averageOrderValue:number}[] }
...
function StudioTourismCard({ source }: { source: StudioTourism }) {
  ...
  return <Card><CardContent className="space-y-5">
    ...
    {source.localMarket?<div className="rounded-lg border p-3 text-sm"><p className="font-medium">Local market: {source.localMarket.name}</p><p className="mt-1 text-xs text-muted-foreground">{source.localMarket.definition} Its {source.localMarket.zipCodes.length} configured ZIP codes are treated as local rather than tourism. This removes {source.localMarket.excludedOrders.toLocaleString()} orders ({money.format(source.localMarket.excludedSales)}) in the selected period.</p><details className="mt-2 text-xs text-muted-foreground"><summary className="cursor-pointer text-foreground">View configured ZIP codes</summary><p className="mt-2 break-words">{source.localMarket.zipCodes.join(", ")}</p><p className="mt-2">Source: {source.localMarket.source}</p></details></div>:null}
    {source.states.length?<fieldset className="rounded-lg border p-3"><legend className="px-1 text-sm font-medium">Optional additional state exclusions</legend><p className="mb-2 text-xs text-muted-foreground">Excluded states remain in ZIP-known totals but are treated like local-market orders, not tourism. Configured local-market ZIPs are already excluded above.</p><div className="flex flex-wrap gap-x-4 gap-y-2">{source.states.map(row=><label key={row.state} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={excluded.includes(row.state)} onChange={()=>toggle(row.state)}/><span>{row.state}</span><span className="text-xs text-muted-foreground">{row.orderCount.toLocaleString()} orders</span></label>)}</div>{excluded.length?<button type="button" className="mt-3 rounded border px-2 py-1 text-xs hover:bg-muted" onClick={()=>setExcluded([])}>Clear exclusions</button>:null}</fieldset>:null}
...
export function TourismDashboard() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardContent className="text-sm text-muted-foreground"><p><strong className="text-foreground">How to read this:</strong> An order is a tourism indicator when its billing ZIP maps to a different state than the studio and is not part of a configured local market. This is not a guest or seat count; one order may contain multiple seats, and travelers may use an in-state billing address.</p></CardContent></Card>
