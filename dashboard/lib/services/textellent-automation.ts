...
export async function getTextellentAutomation(organizationId: number, studioIds: number[]) {
  const [studiosResult, accountsResult, assignmentsResult, settingsResult, deliveriesResult] = await Promise.all([
    ...
    supabase.from("low_reservation_class_alert_settings").select("studio_id,enabled,minimum_reservations,lead_hours,earliest_send_time,message_template,excluded_class_types,excluded_title_patterns,updated_at").eq("organization_id", organizationId).in("studio_id", studioIds),
