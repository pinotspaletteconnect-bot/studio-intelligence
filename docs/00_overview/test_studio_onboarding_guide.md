...
Ask Jeff to confirm your studio name, business name, preferred email address, and the systems you want to connect. He will provide the current SASHA website address and your initial access details through the agreed private channel.

**If you already used SASHA in another workspace:** your existing login can be assigned to your new business without issuing another invitation. Sign in with your current email and SASHA password. The operator must verify that your previous membership and studio grants have been removed and that your new workspace is the one displayed. You may be asked to accept the current legal documents for the new business.
...
If you receive a temporary password, use it within 24 hours. If it expires, ask Jeff to issue a replacement. Do not create a second account to work around a sign-in problem.

**Finished when:** you have the correct website address and access to your own workspace.

## 2. Gather what you need

Start with PTS for operations reporting. Connect other systems only if your studio uses them and Jeff has confirmed they are included in your test. You do not need to purchase another service to finish onboarding.
...
For Google and Meta, SASHA obtains the token through their authorization screens. You do not need to create a developer app, copy a token, or give Jeff your Google or Facebook password.

**Finished when:** you can sign in to each selected vendor and see the correct studio or account.

## 3. Secure your SASHA account

1. Open the SASHA address Jeff provided and sign in.
...
If this section is unavailable, contact Jeff. If you already have a PTS account listed, use **Settings → Integration setup → PTS** to supply or replace its credentials instead of adding a duplicate.
...
If an operator has already created your studio, confirm its name, city, state, and time zone in the studio selector and Workspace Setup. Do not add it again. Once the existing-studio mapping form is released, use **Workspace Setup → Connect an existing studio to PTS** to select your saved PTS account and enter the PTS location ID. Jeff will assist with this step until the release is live. Jennifer should follow this path for Huntington Beach.
...
4. Enter the numeric PTS location ID. This is not your SASHA studio code. The current form points to the PTS location selector or location-specific URL; if you cannot identify the number confidently, ask Jeff to verify it with you. Do not guess.
...
If the brand or PTS dropdown is empty, Jeff needs to complete the missing workspace preparation or help save the account. If a duplicate error appears, review the existing studio instead of creating a differently named duplicate.
...
Open **Settings → Integration setup** (`/settings#integrations`) and expand the relevant system. Complete only the sections you agreed to test. Tell Jeff which systems you skipped; the current app does not provide a complete saved checklist of optional choices.
...
6. Have Jeff confirm the first traffic and attribution import before treating the connection as complete.

If Google blocks access or the expected property is absent, tell Jeff the message and account email without sharing tokens. The application may need test-user eligibility configured; repeated sign-ins will not fix that setting.
...
6. Have Jeff verify the first Ads and Page Insights imports separately. Asset discovery alone does not establish complete Instagram reporting.

If your agency or franchisor controls access, ask them to grant the appropriate access to your account. If Meta blocks the app, Jeff must check app availability and permissions; do not create a personal token as a workaround.
...
4. Save the connection and wait for discovery. Refresh Settings after Jeff confirms discovery has run.
5. Match each intended Eulerity location to its SASHA studio and save the mappings.
6. Have Jeff verify the first spend/performance import. If no locations appear, report that state instead of repeatedly adding accounts.
...
5. Have Jeff validate the first labor import and compare a completed day's hours with Homebase. Review discovered labor roles before relying on cost categories.
...
1. Confirm with Jeff that Textellent is included in this test and what it will be used for.
...
4. Save and ask Jeff to verify the account and sender configuration.
...
5. For each optional integration, compare a representative date in its report with the source account. Agree the history available and the date reporting begins; older history may require a separately arranged backfill.

A studio with no product sales, bookings or classes on a day may have a successful empty report. The current readiness screen can still flag it as missing. Jeff needs to verify the collection result; do not create transactions to turn a badge green.
...
- Confirm with Jeff how password recovery works for this test. Automated recovery depends on email delivery being configured and tested.
...
- Tell Jeff when you change a connected vendor password, revoke access, or see a reconnect request. Credentials must be updated through the secured form or provider sign-in flow.
