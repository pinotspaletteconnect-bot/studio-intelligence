...
backfill route redirected an unauthenticated check to sign-in as expected.
A controlled Huntington Beach workbook upload and warehouse row verification
remain pending because the source workbooks were not available in this session.
No production import was triggered by this release check.
...
passed. Controlled Huntington Beach uploads remain pending.
...
| Accounting automation | n8n is the operator and credential boundary for the private multi-studio accounting workflow. The QuickBooks warehouse foundation remains available for audit/history, and a controlled sandbox run imported 89 accounts, 26 vendors, 68 posted transactions, and 81 lines with writes disabled. QuickBooks setup, OAuth, mapping, and broker routes have been removed from SASHA. Production QuickBooks authorization, four Gmail connections, classification proposals, approvals, attachments, and ledger writes remain pending. |
